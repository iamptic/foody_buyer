  ...
  els.reserveForm.addEventListener('submit', async (ev) => {
    ...
    try {
      ...
      if (data.code) {
        ...
        // Save locally
        try {
          const mine = JSON.parse(localStorage.getItem('foody_res') || '[]');
          mine.unshift({code:data.code, until:data.expires_at, title: currentOffer.title, rest: currentOffer.restaurant});
          localStorage.setItem('foody_res', JSON.stringify(mine.slice(0,10)));
        } catch {}
        load();
      } else {
